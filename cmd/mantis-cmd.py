...
optspec = """
bog mantis [-h server] [-u user] [-p passwd] <outputdir>
--
s,server=   Mantis MySQL server hostname
u,user=     Mantis MySQL server login username
p,passwd=   Mantis MySQL server password
d,dbname=   Mantis MysQL database name
"""
...
for (id, name, email) in \
  query('select id, realname, email from mantis_user_table'):
...
for (id, name) in query('select id, name from mantis_project_table'):
...
for (id, name, projectid) in \
  query('select id,version,project_id from mantis_project_version_table'):
