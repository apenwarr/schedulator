...
namespace Wv.Schedulator
{
	...
	{
	    Log log = new SilentLog("slider");
	    log.log("* {0} + {1}", point, span.TotalHours);
	    ...
	    {
		log.log("W {0} + {1}      ({2})", point, span.TotalHours, hpw);
		...
		log.log("D {0} + {1}      ({2})", point, span.TotalHours, hpd);
	...
	public TimeSpan diff(DateTime x, DateTime y)
	{
	    ...
	    log.log("result: {0} hours", result.TotalHours);
	    return result;
	}
    }
}
